...
function getHtmlTemplate(content, options = {}) {
  const {
    ...
    compactness = 'normal' // 'compact', 'normal', 'spacious'
  } = options;
...
<head>
  ...
  ${includeStyles ? getStyles(theme, compactness) : ''}
</head>
<body>
  ${includePrintButton ? '<button class="print-button" onclick="window.print()">🖨️ Print / Save as PDF</button>' : ''}
...
function getStyles(theme = 'default', compactness = 'normal') {
  const isDark = theme === 'dark';

  // Spacing multipliers based on compactness level
  const spacingMultipliers = {
    compact: 0.5,
    normal: 1,
    spacious: 1.5
  };
  const spacing = spacingMultipliers[compactness] || 1;

  // Line height values based on compactness
  const lineHeights = {
    compact: 1.3,
    normal: 1.6,
    spacious: 1.9
  };
  const lineHeight = lineHeights[compactness] || 1.6;
  ...
  const printFontSize = printFontSizes[compactness] || '11pt';
  ...
  const pageMargin = pageMargins[compactness] || '2cm';
